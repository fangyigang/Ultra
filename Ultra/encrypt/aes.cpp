...
static inline u8 bytex(const u32 x ,const unsigned n)
{
	return x >> (n << 3);
}
...
#define f_rn(bo, bi, n, k) bo[n] =  ft_tab[0][bytex(bi[n],0)] ^ft_tab[1][bytex(bi[(n + 1) & 3],1)] ^  ft_tab[2][bytex(bi[(n + 2) & 3],2)] ^ ft_tab[3][bytex(bi[(n + 3) & 3],3)] ^ *(k + n)

#define i_rn(bo, bi, n, k) bo[n] =  it_tab[0][bytex(bi[n],0)] ^ it_tab[1][bytex(bi[(n + 3) & 3],1)] ^ it_tab[2][bytex(bi[(n + 2) & 3],2)] ^ it_tab[3][bytex(bi[(n + 1) & 3],3)] ^ *(k + n)

#define ls_box(x) ( fl_tab[0][bytex(x, 0)] ^ fl_tab[1][bytex(x, 1)] ^ fl_tab[2][bytex(x, 2)] ^ fl_tab[3][bytex(x, 3)] )

#define f_rl(bo, bi, n, k) bo[n] = fl_tab[0][bytex(bi[n],0)] ^ fl_tab[1][bytex(bi[(n + 1) & 3],1)] ^ fl_tab[2][bytex(bi[(n + 2) & 3],2)] ^  fl_tab[3][bytex(bi[(n + 3) & 3],3)] ^ *(k + n)

#define i_rl(bo, bi, n, k) bo[n] = il_tab[0][bytex(bi[n],0)] ^ il_tab[1][bytex(bi[(n + 3) & 3],1)] ^ il_tab[2][bytex(bi[(n + 2) & 3],2)] ^ il_tab[3][bytex(bi[(n + 1) & 3],3)] ^ *(k + n)
